...
export class WorkoutService {
  ...
  async makeWorkoutPlanForUser(userId: number) {
    ...
    const calculateLoad = (exercise: any) => {
      if (exercise.types === 'weight') {
        // Try to match the second parenthesis group ending in "kg"
        // e.g. "– (80 kg)" ➔ match[1] === "80"
        const m = exercise.name.match(/-\s*\(\s*(\d+(?:\.\d+)?)\s*kg\s*\)\s*$/i);
    ...
    const exercisesDetailed = workout.exercises.map((we) => {
      ...
      const setsArray = Array.from({ length: totalSets }, (_, i) => {
        ...
        if (ex.types && ex.types.toLowerCase() === 'weight') {
          setEntry.weight_used = we.weight;
        }
        return setEntry;
      });
      return {
  ...
  async getExerciseHistory(user_id: number, exercise_cd: string): Promise<any> {
    ...
    for (const dateStr in groupedByDate) {
      ...
      if (
        records[0].exercise.types &&
        records[0].exercise.types.toLowerCase() === 'weight'
      ) {
  ...
  async getUserExerciseHistory(
    ...
    const progressRecords = await this.databaseService.workout_progress.findMany({
      where: {
        user_id,
        date: {
          gte: startMoment.toDate(), // Greater than or equal to start date
          lte: endMoment.toDate(),   // Less than or equal to end date
        },
      },
      include: {
        exerciseProgress: {
